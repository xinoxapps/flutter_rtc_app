...
public class NativeVideoViewController: NSObject, FlutterPlatformView {
    ...
    private var handler_method: String = ""
    ...
    func handle(call: FlutterMethodCall, result: FlutterResult) -> Void {
        if self.handler_method != call.method {
            print("viewcontroller!", #function, call.method);
            self.handler_method = call.method;
        }
        switch(call.method){
        case "player#setVideoSource":
            let arguments = call.arguments as? [String:Any]
            if let args = arguments {
                let videoPath: String? = args["videoUrl"] as? String
                let drmLicenseUrl: String? = args["drmLicenseUrl"] as? String
                let drmCertificateUrl: String? = args["drmCertificateUrl"] as? String
                let contentId: String? = args["contentId"] as? String
                ...
                if let path = videoPath {
                    ...
                    self.videoView?.configure(videoPath: path, isURL: isUrl, drmLicenseUrl: drmLicenseUrl, drmCertificateUrl:drmCertificateUrl, contentId:contentId)
                }
            }
            result(nil)
            break
    ...
    func onFailed(message: String){
        print("viewcontroller", #function, message);
        var arguments = Dictionary<String, Any>()
